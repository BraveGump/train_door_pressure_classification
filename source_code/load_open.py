...
def visualize(feat, labels, epoch, acc, unseen, mis):
    ...
    plt.savefig('/home/user/sun_xin/QE/pm_test/open/images_2/epoch=%d.jpg' % epoch)
    plt.draw()
    plt.pause(0.001)
...
def main():
    ...
    data_test = np.load('all_open_test_2.npy')
    #print(data_test.shape)

    model = torch.load('saved_model_open.pt').to(device)
    centers = torch.load('centers_open.txt').to(device)
    data_train = np.load('open_train.npy')
