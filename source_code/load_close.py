...
def visualize(feat, labels, epoch, acc, unseen, mis):
    ...
    plt.savefig('/home/user/sun_xin/QE/pm_test/close/images_2/epoch=%d.jpg' % epoch)
    plt.draw()
    plt.pause(0.001)
...
def main():
    ...
    data_test = np.load('all_close_test_2.npy')
    #print(data_test.shape)

    model = torch.load('saved_model_close.pt').to(device)
    centers = torch.load('centers_close.txt').to(device)
    data_train = np.load('close_train.npy')
